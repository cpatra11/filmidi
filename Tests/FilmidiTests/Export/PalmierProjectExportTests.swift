...
@Suite("Filmidi project export (collect media)")
struct FilmidiProjectExportTests {
    ...
    private func makeFixture() throws -> (root: URL, source: URL, dest: URL, externalContents: String) {
        ...
        let source = root.appendingPathComponent("Source.filmidi", isDirectory: true)
        ...
        return (root, source, root.appendingPathComponent("Packaged.filmidi", isDirectory: true), externalContents)
    }
    ...
    @Test func collectsExternalAndInternalRewritesSourcesReportsMissing() throws {
        ...
        let report = try FilmidiProjectExporter.export(
