...
enum CIKernelLoader {
    private static func metallibURL(_ lib: String) -> URL? {
        ...
        let candidates = [
            ...
            Bundle.main.resourceURL?.appendingPathComponent("FilmidiPro_FilmidiPro.bundle/\(lib).metallib"), // swift run
            buildDir.appendingPathComponent("FilmidiPro_FilmidiPro.bundle/\(lib).metallib"),                 // swift test
        ].compactMap { $0 }
