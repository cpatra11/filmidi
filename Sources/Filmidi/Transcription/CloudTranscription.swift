...
enum CloudTranscription {
    static func transcribe(
        ...
        if account.mode == .direct, let apiKey = account.qwenAPIKey {
            let audioURL = tempAudioURL.absoluteString
            let submitted = try await DirectTranscriptionBackend.submit(
                audioURL: audioURL,
                durationSeconds: durationSeconds,
                language: language,
                apiKey: apiKey
            )
            let result = try await DirectTranscriptionBackend.waitForResult(
                taskId: submitted.jobId,
                apiKey: apiKey
            ).offsetting(by: range?.lowerBound ?? 0)
            await TranscriptCache.shared.storeCloudTranscript(
                result, for: fileURL, range: range, language: language
            )
            return result
        } else {
            let audioURL = try await GenerationBackend.uploadReference(fileURL: tempAudioURL, contentType: "audio/wav")
