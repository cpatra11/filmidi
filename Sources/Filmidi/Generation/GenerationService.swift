...
final class GenerationService {
    ...
    private static func contentType(for url: URL, fallback: ClipType) -> String {
        ...
        default:
            ...
            case .sequence: return "application/octet-stream"
            }
        }
    }
    ...
    private func monitorBackendJob(
        ...
        while !Task.isCancelled {
            do {
                ...
                if QwenAccountService.shared.mode == .direct, let apiKey = QwenAccountService.shared.qwenAPIKey {
                    job = try await DirectGenerationBackend.pollTask(taskId: backendJobId, apiKey: apiKey)
                } else {
                    job = try await GenerationBackend.pollTask(taskId: backendJobId)
                }
