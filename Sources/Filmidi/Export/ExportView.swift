...
enum ExportDestination: String, CaseIterable, Identifiable {
    ...
    case filmidiProject = "Filmidi Project"
...
struct ExportView: View {
    ...
    @State private var filmidiResult: String?
    @State private var filmidiSummary: (collect: Int, missing: Int, bytes: Int64) = (0, 0, 0)
    ...
    var body: some View {
        ...
        .task {
            filmidiSummary = computeFilmidiSummary()
        }
    }
    ...
    private var filmidiProjectSettings: some View {
        VStack(alignment: .leading, spacing: AppTheme.Spacing.xs) {
            Text("Saves a copy of this project with all media bundled inside, so it opens on any machine.")
                .font(.system(size: AppTheme.FontSize.sm))
                .foregroundStyle(AppTheme.Text.secondaryColor)

            if filmidiSummary.missing > 0 {
                Text("\(filmidiSummary.missing) media file\(filmidiSummary.missing == 1 ? "" : "s") missing - they'll be skipped.")
                    .font(.system(size: AppTheme.FontSize.xs))
                    .foregroundStyle(AppTheme.Status.errorColor)
            }
        }
            ...
            HStack(spacing: AppTheme.Spacing.lg) {
                ...
                case .filmidiProject:
                    HStack(spacing: AppTheme.Spacing.xs) {
                        ...
                        Text("~\(ByteCountFormatter.string(fromByteCount: filmidiSummary.bytes, countStyle: .file))")
                    }
    ...
    private var exportFormat: ExportFormat {
        ...
        case .filmidiProject: .xml   // Filmidi Project has its own path; never rendered.
        case .video: codec.exportFormat
        }
    }

    /// Quick estimate for exporting a Filmidi Project
    private func computeFilmidiSummary() -> (collect: Int, missing: Int, bytes: Int64) {
    ...
    private func startExport() {
        if destination == .filmidiProject { startFilmidiExport(); return }
    ...
    private func startFilmidiExport() {
        filmidiResult = nil
        let panel = NSSavePanel()
        ...
        panel.begin { response in
            guard response == .OK, let url = panel.url else { return }
            Task {
                let report = await service.exportFilmidiProject(
                    timeline: editor.timeline,
                ...
                if report.missing.isEmpty {
                    ...
                    filmidiResult = "Exported, but \(report.missing.count) media file\(report.missing.count == 1 ? "" : "s") were missing and couldn't be included."
                }
            }
        }
    }
