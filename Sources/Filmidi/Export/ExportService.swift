...
final class ExportService {
    ...
    /// Writes a self-contained `.filmidi` bundle (all media collected internally).
    @discardableResult
    func exportFilmidiProject(
        timeline: Timeline,
        ...
        outputURL: URL,
        acquireSlot: Bool = true
    ) async -> FilmidiProjectExporter.Report? {
        isExporting = true
        progress = 0
        error = nil
        lastReport = nil
        ...
        do {
            Log.export.notice(
                "filmidi export start url=\(outputURL.lastPathComponent)",
                telemetry: "Filmidi project export started",
                data: [
                    "tracks": timeline.tracks.count,
                    "clips": timeline.tracks.reduce(0) { $0 + $1.clips.count },
            ...
            let report = try await Task.detached(priority: .userInitiated) {
                try FilmidiProjectExporter.export(
            ...
            progress = 1.0
            Log.export.notice(
                "filmidi export ok collected=\(report.collected.count) missing=\(report.missing.count)",
                telemetry: "Filmidi project export finished",
            ...
            self.error = Log.detail(error)
            Log.export.error(
                "filmidi export failed: \(Log.detail(error))",
                telemetry: "Filmidi project export failed",
