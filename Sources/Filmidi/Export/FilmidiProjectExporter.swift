...
/// Writes a self-contained `.filmidi` package: every resolvable media reference is copied
/// into the new bundle's `media/` directory and rewritten to a project-relative source
enum FilmidiProjectExporter {
    ...
    static func export(
        ...
        let staging = fm.temporaryDirectory.appendingPathComponent("filmidi-export-\(UUID().uuidString)", isDirectory: true)
