...
struct MusicTab: View {
    ...
    private func supportsVideoMode(_ m: AudioModelConfig) -> Bool {
        m.category == .music && m.inputs.contains(.video)
    }

    private var effectiveMode: MusicGenerationSubmission.Mode {
        guard let m = model else { return mode }
        let hasVideo = supportsVideoMode(m)
        let hasText = supportsTextMode(m)
        if hasVideo && hasText { return mode }
        if hasVideo { return .videoToMusic }
        return .textToMusic
    }
    ...
    private var validationNote: String? {
        ...
        if let cost = estimatedCost, cost > AccountService.shared.remainingCredits,
           AccountService.shared.remainingCredits != .max {
    ...
    private var sourceSection: some View {
        InspectorSection("Source") {
            let hasVideo = model.map(supportsVideoMode) ?? false
            let hasText = model.map(supportsTextMode) ?? false
            if hasVideo && hasText {
                ...
                InspectorRow(icon: "slider.horizontal.3", label: "Input") {
                    menuValueLabel("Video to Music")
                }
            } else if hasText {
                InspectorRow(icon: "slider.horizontal.3", label: "Input") {
                    menuValueLabel("Text to Music")
                }
            }
            if isTextMode {
                InspectorRow(
    ...
    private var promptSection: some View {
        InspectorSection(model?.promptLabel ?? "Prompt") {
            ...
            if let model, model.supportsInstrumental {
            ...
            if let model, model.supportsLyrics {
                TextField("Lyrics (optional). [Verse] and [Chorus] tags supported.", text: $lyrics, axis: .vertical)
                    .textFieldStyle(.plain)
                    .lineLimit(2...4)
                    .font(.system(size: AppTheme.FontSize.sm))
                    .foregroundStyle(AppTheme.Text.primaryColor)
    ...
    private func generate() {
        ...
        let theLyrics = model.supportsLyrics && !lyrics.trimmingCharacters(in: .whitespaces).isEmpty ? lyrics : nil
        ...
        if isTextMode {
            ...
            submission = MusicGenerationSubmission(
                ...
                lyrics: theLyrics, instrumental: model.supportsInstrumental ? instrumental : false,
