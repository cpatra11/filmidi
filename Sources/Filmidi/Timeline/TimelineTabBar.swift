...
struct TimelineTabBar: View {
    ...
    var body: some View {
        TimelineTabBarContent(
            editor: editor,
            tabs: editor.openTimelineIds.compactMap { id -> TimelineTabInfo? in
                guard let t = editor.timeline(for: id) else { return nil }
                return TimelineTabInfo(id: t.id, name: t.name)
            },
