...
final class AgentService {
    ...
    isolated deinit {
        ...
        if let token = modeObserver {
            NotificationCenter.default.removeObserver(token)
        }
    }

    var hasApiKey: Bool { QwenAccountService.shared.isConfigured }

    var canStream: Bool { hasApiKey }

    var availableModels: [AnthropicModel] {
        QwenAccountService.shared.canUseDirectAPI ? AnthropicModel.allCases : QwenAccountService.shared.isConfigured ? AnthropicModel.allCases : []
    }
    ...
    private func selectClient() -> (any AgentClient)? {
        ...
        switch account.mode {
        case .direct:
            guard let key = account.qwenAPIKey, !key.isEmpty else { return nil }
            return AnthropicClient(apiKey: key, model: effectiveModel)
        case .backend:
            guard let sessionToken = account.sessionToken,
                  !sessionToken.isEmpty else { return nil }
            return FilmidiClient(backendURL: account.hardcodedBackendURL, sessionToken: sessionToken, model: effectiveModel)
        }
    }
