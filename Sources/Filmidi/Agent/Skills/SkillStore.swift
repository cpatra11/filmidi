...
final class SkillStore {
    ...
    /// Copies under a `filmidi-` prefix so we only overwrite our own prior copy
    @discardableResult
    func copy(_ skill: Skill, to agent: SkillExternalAgent) -> URL? {
        let source = skill.path.deletingLastPathComponent()
        let dest = agent.skillsDirectory.appendingPathComponent("filmidi-\(skill.id)", isDirectory: true)
        let fm = FileManager.default
        do {
