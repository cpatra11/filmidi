...
extension ToolExecutor {
    ...
    private func defaultModelId(_ ids: [(id: String, paidOnly: Bool)], kind: String) throws -> String {
        ...
        guard let match = ids.first(where: { modelAvailable(paidOnly: $0.paidOnly) }) else {
            throw ToolError("No \(kind) model is available on the current plan. Tell the user to configure an API key or upgrade.")
        }
        return match.id
    }
    ...
    func generate(_ editor: EditorViewModel, _ args: [String: Any], type: ClipType) throws -> ToolResult {
        ...
        guard AccountService.shared.isSignedIn else {
            throw ToolError("Generation requires an API key. Tell the user to configure their Qwen key in Settings, or subscribe for backend access.")
        }
        guard AccountService.shared.hasCredits else {
            throw ToolError("Out of credits. Tell the user to add credits or subscribe.")
        }
        switch type {
        case .video:
    ...
    func generateAudio(_ editor: EditorViewModel, _ args: [String: Any]) async throws -> ToolResult {
        guard AccountService.shared.isSignedIn else {
            throw ToolError("Audio generation requires an API key. Tell the user to configure their Qwen key in Settings, or subscribe for backend access.")
        }
        guard AccountService.shared.hasCredits else {
            throw ToolError("Out of credits. Tell the user to add credits or subscribe.")
        }
    ...
    func upscaleMedia(_ editor: EditorViewModel, _ args: [String: Any]) throws -> ToolResult {
        ...
        guard AccountService.shared.isSignedIn else {
            throw ToolError("Upscale requires an API key. Tell the user to configure their Qwen key in Settings, or subscribe for backend access.")
        }
        guard AccountService.shared.hasCredits else {
            throw ToolError("Out of credits. Tell the user to add credits or subscribe.")
        }
