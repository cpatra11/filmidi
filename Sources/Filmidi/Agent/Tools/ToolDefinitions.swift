...
enum ToolDefinitions {
        ...
        AgentTool(
            ...
            inputSchema: objectSchema(
                properties: [
                    ...
                    "segments": ["type": "string", "description": "Sonilo models only. Optional JSON array of timed style segments, e.g. [{\"start\":0,\"prompt\":\"calm ambient\"},{\"start\":30,\"prompt\":\"energetic drum and bass\"}]. Each segment has 'start' (seconds, 0-based) and 'prompt' (style description)."],
                    "folderId": ["type": "string", "description": "Optional. Folder id (from list_folders or create_folder) to place the result in. Omit for the project root."],
                ],
                required: []
            )
                ...
                properties: [
                    "source": [
                        ...
                        "properties": [
                            ...
                            "path": ["type": "string", "description": "Absolute local file or directory path, readable by the Filmidi process. A directory is imported recursively — every openable file is pulled in and the folder structure is replicated as media folders."],
    ...
    static let openProject = AgentTool(
        name: .openProject,
        description: "Open a project and make it the active one — every editing tool then acts on it. Identify it by `id` (from get_projects) or by `path` to a .filmidi package. If it's already open, it's brought to front. Returns the now-active project and how many are open. The user sees the window change.",
        inputSchema: objectSchema(
            properties: [
                "id": ["type": "string", "description": "Project id from get_projects. Provide this or path."],
                "path": ["type": "string", "description": "Filesystem path to a .filmidi package. Provide this or id."],
            ]
        )
    )
    ...
    static let newProject = AgentTool(
        name: .newProject,
        description: "Create a new empty project in the user's Filmidi Pro folder and make it active. Fails if a project with that name already exists — pick another name. Returns the new project's name and path.",
        inputSchema: objectSchema(
            properties: [
