...
extension ToolExecutor {
    func exportProject(_ editor: EditorViewModel, _ args: [String: Any]) async throws -> ToolResult {
        ...
        case .filmidi:
            return try await exportFilmidi(editor, outputURL: outputURL)
        }
    }
    ...
    private func exportFilmidi(_ editor: EditorViewModel, outputURL: URL) async throws -> ToolResult {
        ...
        let service = ExportService()
        guard let report = await service.exportFilmidiProject(
            ...
            throw ToolError("export_project: \(service.error ?? "Filmidi project export failed")")
        }
        ...
        return try jsonResult([
            ...
            "mode": ExportProjectMode.filmidi.rawValue,
            "path": outputURL.path,
    ...
    private func directExportURL(_ path: String, mode: ExportProjectMode, format: ExportFormat?) throws -> URL {
        ...
        if fm.fileExists(atPath: expanded, isDirectory: &isDirectory),
           ...
           !(mode == .filmidi && rawExtension == expectedExtension) {
...
private enum ExportProjectMode: String {
    ...
    init(named raw: String?) throws {
        ...
        guard let mode = Self(rawValue: raw.normalizedExportOption) else {
            throw ToolError("export_project: mode must be video, xml, fcpxml, or filmidi")
        }
        self = mode
    }
    ...
    func fileExtension(format: ExportFormat?) -> String {
        ...
        case .xml: "xml"
        case .fcpxml: "fcpxml"
        case .filmidi: Project.fileExtension
        }
    }
